...
screen choice(items):
    tag menu
    zorder 150    # keep choices above overlays while debugging


    # Split caption and choices.
    $ caption = None
    $ choices = []
    for i in items:
        if caption is None and i.action is None:
            $ caption = i.caption
        elif i.action is not None:
            $ choices.append(i)

    # # Quick on-screen debug so we know this screen ran.
    # $ renpy.notify("CHOICE debug: %r  (%d choices)" % (caption, len(choices)))
    ...
    window:
        ...
        vbox:
            ...
            if caption:
                text caption style "menu_caption" xalign 0.0   # top/left of box

            for c in choices:
                textbutton c.caption action c.action style "choice_button" xalign 0.0
...
style main_menu_title:
    size 80
    color "#F2E6D2"
    outlines [(4, "#000000CC", 0, 0), (8, "#2A0E12AA", 0, 0)]
    kerning 1
